...
if (number) {
  (async () => {
    const pokemon = await pokeApi.getPokemonDetail({ url: `https://pokeapi.co/api/v2/pokemon/${number}/` });

    // Dados da espécie
    const speciesRes = await fetch(pokemon.speciesUrl);
    const speciesData = await speciesRes.json();

    const categoryFull = speciesData.genera.find(g => g.language.name === 'en').genus;
    const category = categoryFull.replace(' Pokémon', '');

    const gender = speciesData.gender_rate === -1 ? 'Desconhecido' : '♂ / ♀';
